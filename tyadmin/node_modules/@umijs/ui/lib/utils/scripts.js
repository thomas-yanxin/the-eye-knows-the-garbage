...
function _macaddress() {
  const data = _interopRequireDefault(require("macaddress"));

  _macaddress = function _macaddress() {
    return data;
  };
...
const cheerio = _umi().utils.cheerio; // 添加埋点脚本


function normalizeHtml(html, scripts) {
  const bigfishScripts = scripts.bigfishScripts,
        umiScripts = scripts.umiScripts;
  const $ = cheerio.load(html); // basementMonitor

  $('head').append('<meta name="bm_app_id" content="5d68ffc1d46d8743e5445b68">');
  $('body').append(`<script>window.g_umi = { version: "${process.env.UMI_VERSION || ''}"};</script>`);

  if (process.env.BIGFISH_COMPAT) {
    $('body').append(`<script>window.g_bigfish = { version: "${process.env.BIGFISH_VERSION || ''}" };</script>`);
...
const getMacId = /*#__PURE__*/function () {
  var _ref = _asyncToGenerator(function* () {
    ...
    return new Promise(resolve => {
      _macaddress().default.one((err, mac) => {
        macId = err ? '' : mac;
        resolve(macId);
      });
    });
...
const render = (template, model) => template.replace(/{{ (\w+) }}/g, (str, key) => model[key]);

const getScripts = /*#__PURE__*/function () {
  var _ref2 = _asyncToGenerator(function* () {
    const macAddress = yield getMacId();
    const modal = {
      roleId: macAddress,
