...
const writeRoutes = /*#__PURE__*/function () {
  var _ref = _asyncToGenerator(function* (ctx, args) {
    ...
    if (generator.needCreateNewRoute && api.userConfig.routes && !skipModifyRoutes) {
      ...
      debug('api.service.configInstance.configFile', configFile); // 当前 _modifyBlockNewRouteConfig 只支持配置式路由
      // 未来可以做下自动写入注释配置，支持约定式路由

      const newRouteConfig = yield api.applyPlugins({
      ...
      try {
        if (!dryRun) {
          (0, _blockSdk().writeNewRoute)(newRouteConfig, configFile, api.paths.absSrcPath);
        }
      } catch (e) {
    ...
    if (!generator.isPageBlock) {
      ...
      (0, _blockSdk().appendBlockToContainer)({
