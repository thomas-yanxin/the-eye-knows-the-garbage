...
function _getConfig() {
  _getConfig = _asyncToGenerator(function* (opts) {
    ...
    webpackConfig.module.rule('js').test(/\.(js|mjs|jsx|ts|tsx)$/).include.add(cwd).end().exclude.add(/node_modules/).end().use('babel-loader').loader(require.resolve('babel-loader')).options(babelOpts); // prettier-ignore
    ...
    if (nodeModulesTransform.type === 'all') {
      ...
      const pkgs = _objectSpread({}, (0, _nodeModulesTransform.es5ImcompatibleVersionsToPkg)(), {}, (0, _nodeModulesTransform.excludeToPkgs)({
    ...
    webpackConfig.plugin('copy').use(require.resolve('copy-webpack-plugin'), [[(0, _fs().existsSync)((0, _path().join)(cwd, 'public')) && {
      from: (0, _path().join)(cwd, 'public'),
      to: absOutputPath
    }, ...(config.copy ? config.copy.map(from => ({
      from: (0, _path().join)(cwd, from),
      to: absOutputPath
    })) : [])].filter(Boolean)]); // timefix
    // webpackConfig
