...
function parallelizeBrowserTests(tests) {
  ...
  (0, _nightmare.default)(config).goto("https://example.com").evaluate( // eslint-disable-next-line no-eval
...
async function astMetarataInfererTester(apiMetadata) {
  ...
  return Promise.all([parallelizeBrowserTests(supportedApis.map(e => getJsAssertions(e).determineASTNodeTypes)), parallelizeBrowserTests(supportedApis.map(e => getJsAssertions(e).determineIsStatic))]).then(([astNodeTypeTestResults, isStaticTestResults]) => supportedApis.map((e, i) => ({ ...e,
