...
function createTypeScriptReporter(configuration) {
    ...
    const performance = TypeScriptPerformance_1.connectTypeScriptPerformance(Performance_1.createPerformance());
    ...
    return {
        getReport: ({ changedFiles = [], deletedFiles = [] }) => __awaiter(this, void 0, void 0, function* () {
            ...
            if (!system) {
                system = ControlledTypeScriptSystem_1.createControlledTypeScriptSystem(configuration.mode);
            }
            ...
            if (!parsedConfiguration) {
                ...
                parsedConfiguration = TypeScriptConfigurationParser_1.parseTypeScriptConfiguration(configuration.configFile, configuration.context, configuration.configOverwrite, parseConfigFileHost);
                ...
                if (parseConfigurationDiagnostics.length) {
                    parsedConfiguration = undefined;
                    let issues = TypeScriptIssueFactory_1.createIssuesFromTsDiagnostics(parseConfigurationDiagnostics);
                ...
                if (configurationChanged) {
                    ...
                    if (typeof ts.getTsBuildInfoEmitOutputFilePath === 'function' &&
                        ...
                        parsedConfiguration.options.incremental) {
                        const tsBuildInfoPath = ts.getTsBuildInfoEmitOutputFilePath(parsedConfiguration.options);
                        if (tsBuildInfoPath) {
                            try {
            ...
            if (configuration.build) {
                ...
                if (!watchSolutionBuilderHost) {
                    ...
                    watchSolutionBuilderHost = ControlledWatchSolutionBuilderHost_1.createControlledWatchSolutionBuilderHost(parsedConfiguration, system, ts.createSemanticDiagnosticsBuilderProgram, undefined, undefined, undefined, undefined, (builderProgram) => {
                ...
                if (!solutionBuilder) {
                    ...
                    solutionBuilder = ts.createSolutionBuilderWithWatch(watchSolutionBuilderHost, [configuration.configFile], {});
                ...
                if (!watchCompilerHost) {
                    ...
                    watchCompilerHost = ControlledWatchCompilerHost_1.createControlledWatchCompilerHost(parsedConfiguration, system, ts.createSemanticDiagnosticsBuilderProgram, undefined, undefined, (builderProgram) => {
                        const projectName = getProjectNameOfBuilderProgram(builderProgram);
                        const diagnostics = getDiagnosticsOfBuilderProgram(builderProgram);
                        // update diagnostics
                        diagnosticsPerProject.set(projectName, diagnostics);
            ...
            let issues = TypeScriptIssueFactory_1.createIssuesFromTsDiagnostics(diagnostics);
            extensions.forEach((extension) => {
                if (extension.extendIssues) {
                    issues = extension.extendIssues(issues);
                }
