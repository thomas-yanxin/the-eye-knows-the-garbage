...
function createControlledTypeScriptSystem(mode = 'readonly') {
    // watchers
    ...
    function updateDirectorySnapshot(path, recursive = false) {
        const dirents = passiveFileSystem.readDir(path);
        if (!directorySnapshots.has(path)) {
            directorySnapshots.set(path, dirents.filter((dirent) => dirent.isFile()).map((dirent) => path_1.join(path, dirent.name)));
        }
        if (recursive) {
            dirents
                .filter((dirent) => dirent.isDirectory())
                .forEach((dirent) => updateDirectorySnapshot(path_1.join(path, dirent.name)));
        }
    }
    ...
    const controlledSystem = Object.assign(Object.assign({}, ts.sys), { useCaseSensitiveFileNames: caseSensitive, fileExists(path) {
        ...
        pollAndInvokeCreatedOrDeleted() {
            ...
            function diffDirectorySnapshots(prevFiles, nextFiles) {
            ...
            prevDirectorySnapshots.forEach((prevFiles, path) => diffDirectorySnapshots(prevFiles, directorySnapshots.get(path)));
            directorySnapshots.forEach((nextFiles, path) => diffDirectorySnapshots(prevDirectorySnapshots.get(path), nextFiles));
